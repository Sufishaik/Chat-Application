    ...
    const searchContacts = async (searchTerm) => {
        try {
            if (searchTerm.length > 0) {
                const resp = await fetch('https://chat-application-clit.onrender.com/api/auth/searchTerm', {
                    method: 'POST',
                    credentials: 'include',
                    headers: {
                                            ...
                                            <div onClick={() => handleContactSelect(item)} className="flex gap-4 mb-5 items-center cursor-pointer">
                                                <div className="w-12 h-12 relative">
                                                    <Avatar className="h-12 w-12 rounded-full overflow-hidden">
                                                        {
                                                            item?.image ? <AvatarImage className="object-cover w-full h-full bg-black " src={`https://chat-application-clit.onrender.com/${item?.image}`} /> : <div className="uppercase h-12 w-12 text-lg border-[1px] flex items-center justify-center">{item?.firstName ? item?.firstName.split("").shift() : item?.email?.split("").shift()}</div>
                                                        }
                                                    </Avatar>
                                                </div>
