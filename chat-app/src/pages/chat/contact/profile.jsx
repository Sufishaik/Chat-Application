    ...
    const handleLogout = async () => {
        try {
            const resp = await axios.post(
                'https://chat-application-clit.onrender.com/api/auth/logout',
                {},
        ...
        <>
            <div className="flex h-15 items-center justify-center mx-[0px auto] pr-10 gap-4 p-2 bg-[#2a2b33] w-full pl-10 absolute bottom-0 cursor-pointer">
                <Avatar className="h-12 w-12  rounded-full overflow-hidden">
                    {
                        userInfo.image ? <AvatarImage src={`https://chat-application-clit.onrender.com/${userInfo.image}`} className='object-cover w-full h-full bg-black' /> : <div className={`uppercase h-12 w-12 text-lg boder-[1px] flex items-center justify-center `}>
